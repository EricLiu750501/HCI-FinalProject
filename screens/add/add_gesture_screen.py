...
class AddGestureScreen(BaseScreen):
    ...
    def __init__(self, callback):
        ...
        self.nod_frequency_range = (
            2,
            8,
        )  # 點頭頻率範圍（以 Hz 為單位，假設每秒 30 幀）
        ...
        if success:
                ...
                for face_landmarks in face_results.multi_face_landmarks:
                    ...
                    for i in range(len(self.landmark_sections)):
                        ...
                        for ele in tmp_set:
                            ...
                            tmp_lstx.append(x)
                            tmp_lsty.append(y)
                        self.brow_x_positions[self.frame_count_i] = int(
                            sum(tmp_lstx) / len(tmp_lstx)
                        )
                        self.brow_y_positions[self.frame_count_i] = int(
                            sum(tmp_lsty) / len(tmp_lsty)
                        )  # mean point of brows
            ...
            if True:  # 可能會有一些條件
                ...
                smoothed_positions = np.convolve(
                    self.brow_y_positions, np.ones(5) / 5, mode="valid"
                )  # 平滑數據
                fft_signal = np.fft.fft(smoothed_positions)
                amplitude = np.abs(fft_signal)[1:]  # 30fps , 忽略 0Hz
                frequencies = np.fft.fftfreq(len(smoothed_positions), 1 / 30)[
                    1:
                ]  # 30fps , 忽略 0Hz
                maximum = 0
                max_ind = 0
                ...
                if (
                    self.nod_frequency_range[0]
                    <= abs(frequencies[max_ind])
                    <= self.nod_frequency_range[1]
                ):
                    self.nod_success += 1
                    if self.debug_mode:  # for debug
                        cv2.putText(
                            frame,
                            "Nod Detected: Harmonic Motion",
                            (50, 50),
        ...
        if hands_results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(
                ...
                for i in range(1, len(landmarks)):
                    ...
                    array[i - 1] = np.linalg.norm(
                        np.array(cur_vector) - np.array(base_vector)
                    )
    ...
    def __tk_get_char(self):
        ...
        entry_zh = tk.Entry(
            frame_zh,
            textvariable=user_input_zh,
            width=50,
            validate="key",
            validatecommand=(validate_command_zh, "%S"),
        )
        ...
        entry_en = tk.Entry(
            frame_en,
            textvariable=user_input_en,
            width=50,
            validate="key",
            validatecommand=(validate_command_en, "%S"),
        )
