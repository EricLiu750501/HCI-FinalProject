...
class ShowScreen(BaseScreen):
    def __init__(self, callback):
        ...
        self.font_path = "assets/fonts/NotoSansTC-Bold.ttf"
        # self.play_sound = pygame.mixer.Sound("assets/sounds/playsound.mp3")
        self.play_sound = pygame.mixer.Sound("assets/sounds/test3.mp3")

        # 載入背景圖片
        self.background = cv2.imread("assets/images/ShowScreenBG.png", cv2.IMREAD_COLOR)
    ...
    def set_jutsu(self, jutsu):
        ...
        if jutsu["id"] > 6:
            self.cur_g_index = (
                -1
            )  # for displaying the 1st frame in __display_consequtive
    ...
    def __display_video(self, frame):
        ...
        if self.cap.isOpened():
            ...
            if ret:
                frame[100 : 100 + 480, 300 : 300 + 640] = cv2.resize(
                    video_frame, (640, 480)
                )
            else:
    ...
    def __display_consequtive_images(self, frame):
        ...
        if self.jutsu["sequence"][self.cur_g_index] <= 12:
            ...
            img_path = (
                f"assets/images/gesture_{self.jutsu['sequence'][self.cur_g_index]}.jpg"
            )
    ...
    def __draw_buttons(self, frame):
        ...
        # draw back button
        back_x, back_y = WINDOW_SIZE[0] - 250, WINDOW_SIZE[1] - 100
